...
public class ArzubovEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timer>=coolDown && player!=null && Rast(arzubov.transform.position.x, arzubov.transform.position.y, player.transform.position.x, player.transform.position.y) <= 30f)
        { 
            Instantiate(bullet, arzubov.transform.position, Quaternion.Euler(0, 0, 0));
            timer = 0;
        }
